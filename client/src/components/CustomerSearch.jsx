...
export default function CustomerSearch({ onSelect, selectedCustomer, onClear }) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const res = await fetch(`${API_BASE}/customers/search?q=${encodeURIComponent(query)}&limit=10`, {
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: '0.85rem', color: '#666', display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            {selectedCustomer.balance > 0 && (
              <span style={{ color: '#16a34a' }}>💰 Dư: {formatPrice(selectedCustomer.balance)}</span>
            )}
            {selectedCustomer.total_debt > 0 && (
              <span style={{ color: '#dc2626' }}>📕 Nợ: {formatPrice(selectedCustomer.total_debt)}</span>
            )}
          </div>
        </div>
        <button
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                {customer.parent_phone && (
                  <span style={{ 
                    ...
                    {customer.relationship || 'Người nhận'}
                  </span>
                )}
              </div>
              <div style={{ 
                ...
                {customer.total_debt > 0 ? (
                  <span style={{ color: '#dc2626', display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <AlertCircle size={14} />
                    Nợ: {formatPrice(customer.total_debt)} ({customer.pending_orders} đơn)
                  </span>
                ) : (
