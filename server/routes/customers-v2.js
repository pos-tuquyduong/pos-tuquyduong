...
router.get('/search/:query', authenticate, async (req, res) => {
  try {
    ...
    // Tìm trong SX
    const sxResults = sxCustomers.filter(c => 
    ...
    const regResults = registrations.filter(r =>
      !sxPhones.has(r.phone) && (r.name?.toLowerCase().includes(q) || r.phone?.includes(q))
    ).map(r => ({
      name: r.name, phone: r.phone,
      balance: walletMap[r.phone]?.balance || 0,
      source: 'pos', is_pending: r.status === 'pending'
    }));
...
router.get('/', authenticate, async (req, res) => {
  try {
    const sxCustomers = await fetchSXCustomers();
    const wallets = query('SELECT * FROM pos_wallets');
    const registrations = query("SELECT * FROM pos_registrations WHERE status = 'pending'");
    ...
    const result = sxCustomers.map(c => {
      ...
      delete regMap[phone];
      return { ...c, phone, balance: wallet?.balance || 0, source: 'sx', is_synced: true };
    });
    ...
    Object.values(regMap).forEach(r => {
      result.push({
        ...
        balance: walletMap[r.phone]?.balance || 0,
        source: 'pos', is_synced: false, is_pending: true,
        registration_id: r.id, requested_product: r.requested_product
      });
    });
...
router.get('/:phone', authenticate, async (req, res) => {
  try {
    ...
    const sxCustomers = await fetchSXCustomers();
    const sxCustomer = sxCustomers.find(c => normalizePhone(c.phone) === phone);
    const wallet = queryOne('SELECT * FROM pos_wallets WHERE phone = ?', [phone]);
    const registration = queryOne('SELECT * FROM pos_registrations WHERE phone = ?', [phone]);
    ...
    if (sxCustomer) {
      ...
      customer = {
        name: registration.name, phone: registration.phone, notes: registration.notes,
        source: 'pos', is_synced: false, is_pending: registration.status === 'pending',
...
router.get('/:phone/full', authenticate, async (req, res) => {
  try {
    ...
    if (sxCustomer) {
      ...
      customer = { name: registration.name, phone, source: 'pos', is_pending: registration.status === 'pending' };
    } else if (wallet) {
      customer = { phone, source: 'pos', is_retail: true };
    } else {
    ...
    const transactions = query('SELECT * FROM pos_balance_transactions WHERE customer_phone = ? ORDER BY created_at DESC LIMIT 20', [phone]);
    const orders = query('SELECT * FROM pos_orders WHERE customer_phone = ? ORDER BY created_at DESC LIMIT 20', [phone]);
