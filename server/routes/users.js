...
router.post('/', authenticate, checkPermission('manage_users'), async (req, res) => {
  try {
    ...
    const existing = await queryOne('SELECT id FROM pos_users WHERE username = ?', [username]);
    if (existing) {
    ...
    const user = await queryOne('SELECT id, username, display_name, role, is_active FROM pos_users WHERE id = ?', [result.lastInsertRowid]);
...
router.put('/:id', authenticate, checkPermission('manage_users'), async (req, res) => {
  try {
    ...
    const user = await queryOne('SELECT * FROM pos_users WHERE id = ?', [req.params.id]);
    if (!user) {
    ...
    const updated = await queryOne('SELECT id, username, display_name, role, is_active FROM pos_users WHERE id = ?', [req.params.id]);
 ...
 * Reset mật khẩu
 */
router.put('/:id/password', authenticate, checkPermission('manage_users'), async (req, res) => {
  try {
    ...
    await run('UPDATE pos_users SET password = ? WHERE id = ?', [hashedPassword, req.params.id]);
 ...
 * Danh sách quyền
 */
router.get('/permissions', authenticate, checkPermission('manage_permissions'), async (req, res) => {
  try {
 ...
 */
router.put('/permissions/:role', authenticate, checkPermission('manage_permissions'), async (req, res) => {
  try {
